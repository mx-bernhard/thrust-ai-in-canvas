...
export class RRTPathPlanner {
  ...
  private readonly minPassageWidth: number = 40; // Minimum width for passages
  ...
  public findPath(start: Vector2D, goal: Vector2D): Vector2D[] {
    ...
    for (let i = 0; i < this.maxIterations; i++) {
      ...
      if (
        !this.collides(nearestNode.position, newPosition) &&
  ...
  private isInNarrowPassage(point: Vector2D): boolean {
    ...
    if (distances.length >= 2) {
      ...
      if (
        distances[0].distance < this.minPassageWidth / 2 &&
        ...
        const closestPoint1 = this.closestPointOnRectangle(
          point,
          firstObstacle,
        );
        const closestPoint2 = this.closestPointOnRectangle(
          point,
          secondObstacle,
        );
  ...
  private sampleRandomPoint(): Vector2D {
    return {
      x:
        this.boundaryMargin +
        Math.random() * (this.width - 2 * this.boundaryMargin),
      y:
        this.boundaryMargin +
        Math.random() * (this.height - 2 * this.boundaryMargin),
    };
  }
  ...
  private collides(from: Vector2D, to: Vector2D): boolean {
    ...
    if (
      to.x < effectiveBoundaryMargin ||
      to.x > this.width - effectiveBoundaryMargin ||
      to.y < effectiveBoundaryMargin ||
      to.y > this.height - effectiveBoundaryMargin
    ) {
      return true;
    }
  ...
  private simplifyPath(path: Vector2D[]): Vector2D[] {
    ...
    while (i < path.length - 1) {
      ...
      for (let j = path.length - 1; j > i; j--) {
        if (
          !this.collides(path[i], path[j]) &&
          !this.hasNarrowPassage(path[i], path[j])
        ) {
          furthestVisible = j;
          break;
        }
